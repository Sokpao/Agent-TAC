...
public class DummyAgent extends AgentImpl {
    ...
    private static final float HOTEL_MAXPRICE = 600; // maxiumum price per room and night (750)
    ...
    private void sendBids() {
        for (int i = 0, n = agent.getAuctionNo(); i < n; i++) {
            ...
            if (price > 0) { // bidding the prices initiallly
                ...
                if (DEBUG) {
                    log.finest("Type of auction: " + agent.getAuctionCategory(i) 
                            + " submitting bid with alloc= " + agent.getAllocation(i) 
                            + " own= " + agent.getOwn(i));
                }
                agent.submitBid(bid);
            }
    ...
    */
    ...
    private void calculateAllocation() {
		...
		// -------------------------------------------------------------
                ...
                log.finer("Adding inflight for day: " + inFlight + " on " + auction);
                agent.setAllocation(auction, agent.getAllocation(auction) + 1);
                
                auction = agent.getAuctionFor(TACAgent.CAT_FLIGHT,
                        TACAgent.TYPE_OUTFLIGHT, outFlight);
                log.finer("Adding outflight for day: " + outFlight + " on " + auction);
                agent.setAllocation(auction, agent.getAllocation(auction) + 1);
		...
		// -------------------------------------------------------------
                ...
                for (int d = inFlight; d < outFlight; d++) {
                    ...
                    log.finer("Adding hotel for day: " + d + " on " + auction + " with type of " + type);
                    agent.setAllocation(auction, agent.getAllocation(auction) + 1);
                }
		...
		// gets the list of events client wants
                ...
                for (int k = 0; k < days.length; k++) { 
                    ...
                    log.finer("Adding entertainment " + eTypes[k] + " on day "+ days[k] +" on auction " + auction);
                    agent.setAllocation(auction, agent.getAllocation(auction) + 1);
                }
            }
        }
